...
var newBadWords = ['faggot', 'fag', 'nigga','nigger', 'rape', 'retarded', 'retard', 'kyke', 'dyke', 'tranny', 'chink', 'trannie', 'testcurse'];
/*https://www.npmjs.com/package/bad-words-plus*/
var Filter = require('bad-words-plus'),
    filter = new Filter({firstLetter: true, lastLetter: true,  emptyList: true});
...
io.on('connection', function(socket) {
  ...
  socket.on('switchRoom', function(newroom) {
    ...
    /*else if(newroom == 'mod'){
      if(socket.admin){
        ...
        newroom = 'main';
        console.log(username+' tried to join mod.');
      }
    }*/
    else {
  ...
  /* BROKEN AUTOKICK/AUTOBAN
if (msg.includes('newBadWords')) {
      var target = username;
  ...
  socket.on('disconnect', (reason) => {
    if (username) {
      ...
      setTimeout(function() {
        ...
        if (username.startsWith("BANNEDUSER") == true){}
        else {
...
/*BROKEN KICK FUNCTION I GUESS */
  socket.on('reqKick', function(user) {
    if (!socket.admin) {
      socket.emit('serverChat', 'Nice try.');
      return;
    }
    ...
    if (user == 'all') {
      io.emit('kick');
      socket.emit('serverChat', 'Successfully kicked all users.');
    } else if (user in usernames) {
      io.to(usernames[user].id).emit('kick',username);
      io.to(room).emit('serverChat', user + ' was kicked.');
      console.log(username+' kicked '+user);
    } else {
      socket.emit('serverToast', 'No user has the name ' + user + '.');
    }
  });
  ...
  socket.on('pm', function(user, msg, img) {
    ...
    if (user in usernames) {
      try {
        msg = anchorme({input:msg,options:{attributes:{target:"_blank"},specialTransform:[{test:/.*\.(png|jpg|gif)$/,transform:s=>`<img src="${s.startsWith("http")?s:`http://${s}`}" class="chat-img">`},{test:/youtube\.com\/watch\?v\=/,transform:t=>`<iframe src="https://www.youtube.com/embed/${t.replace(/.*watch\?v\=(.*)$/,"$1").split('&')[0]}" class="chat-video"></iframe>`}]}});
        msg = notifyMentions(msg);
        if (!socket.admin || socket.admin || admin) msg = filter.clean(msg);
      } catch (e) {}
      ...
      console.log(username+' private messaged '+user+' :'+msg);
    } else {
      socket.emit('serverToast', 'No user has the name ' + user + '.');
    }
    }
  ...
  socket.on('reqReload', function() {
    ...
    // updates clients
    io.emit('sendReload');
    console.log(username+' requested an reload');
  });
